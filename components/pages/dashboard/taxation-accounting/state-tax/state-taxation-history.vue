				...
				<table class="min-w-max w-full flex flex-col gap-px">
					<tr v-for="(data, index) in history" :key="index"
						...
						<td class="w-auto" v-for="(partner, index) in data.partners" :key="index">
							<span v-if="index != 0">,</span> {{ partner.first_name }} {{ partner.middle_name }} {{
									partner.last_name
							}}
						</td>
					</tr>
...
export default {
	...
	methods: {
		...mapActions({
			STATE_TAXATION_HISTORY: "stateTaxation/STATE_TAXATION_HISTORY"
		}),
		taxationHistory() {
			this.loader = true
			this.STATE_TAXATION_HISTORY({ company_id: this.GET_CURRENT_COMPANY.id })
				.then((response) => {
					this.loader = false
					if (response.data['state-tax-history']) {
						this.history = response.data['state-tax-history']
					}
				})
		}
	},

...
export default {
	...
	methods: {
		...
		deletePartner() {
			this.DELETE_PARTNER_DIRECTOR({ company_id: this.GET_CURRENT_COMPANY.id, stateTaxID: this.GET_STATE_ID, partnerID: this.selectedPartner.id })
				.then((response) => {
					this.disableButton = false
					let arr = [];
					this.GET_STATE_STAKEHOLDERS.forEach((partner, index) => {
						if (partner.role === 'director') {
							arr.push(partner)
						}
						else if (partner.role === 'partner' && this.selectedPartner.id !== partner.id) {
							arr.push(partner)
						}
					});
